 ...
 * (SEC-5). Selection: per-product match first, then default flag, then
 * priority — a future load-balancing strategy slots into select_for().
 */
final class Credentials
{
    ...
    public static function select_for(string $product): ?array
    {
        ...
        $fallback = null;
        foreach ($rows as $r) {
            $products = array_filter(explode(',', (string) $r['products']));
            ...
            if ($product !== '' && in_array($product, $products, true)) {
                return $candidate; // explicit product match wins
            }
