...
| Customer-specific commercial rules (markup/discount/limits) | §6, §customer rules | `Customers\Rules`; **`spending_limit` is a checkout gate in `OrderService::create`; `credit_limit` is not — it governs the credit-policy grace→restricted band, not the checkout path** (`OrderService.php:126-141`, comment states this explicitly) | E: `'spending_limit blocks over-limit purchase'`, `'credit_limit does NOT block a gateway order (regression fix)'` | ✅ |
