...
**Controls:** Tokens sodium-encrypted at rest keyed from WP salts (DB dump alone is useless — M3 without file access gets nothing); UI masks to last-4; no REST route returns `token_enc`; `Audit::redact` strips secret-shaped keys from every log row. *Residual: an attacker with BOTH the DB and `wp-config.php` salts can decrypt — equivalent to full site compromise, out of scope.*
