<?php
/**
 * Product configuration + purchase page.
...
<h1 class="arvrs-page-title"><?php echo esc_html($product_label); ?></h1>
        ...
        <span class="arvrs-plan-body">
          ...
          <span class="arvrs-plan-specs">
            <?php foreach ($plan['specs'] as $spec_key => $spec_value) : ?>
              <span class="arvrs-spec"><span class="arvrs-muted"><?php echo esc_html($spec_key); ?>:</span> <?php echo esc_html($spec_value); ?></span>
            <?php endforeach; ?>
          </span>
          <span class="arvrs-plan-price"><?php echo esc_html($plan['price_label']); ?> <span class="arvrs-muted">/ <?php esc_html_e('ماهانه', 'arvan-reseller'); ?></span></span>
        </span>
      </label>
    <?php endforeach; ?>
  </section>

  <aside class="arvrs-order-side">
    <div class="arvrs-card">
      ...
      <?php if ($product === 'cloud_server') : ?>
        ...
        <div class="arvrs-field">
          ...
          <input id="arvrs-name" name="name" type="text" maxlength="50" placeholder="my-server" />
        </div>
      ...
      <div class="arvrs-order-summary">
        <span><?php esc_html_e('مبلغ قابل پرداخت', 'arvan-reseller'); ?></span>
        <strong id="arvrs-total"><?php echo esc_html($plans[0]['price_label'] ?? ''); ?></strong>
      </div>

      <?php if ($logged_in) : ?>
        <button type="submit" class="arvrs-btn arvrs-btn-primary arvrs-btn-block" id="arvrs-buy">
